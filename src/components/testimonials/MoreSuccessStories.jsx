...
function StarRating({ count = 5 }) {
  return (
    <span className="inline-flex gap-0.5 mt-1" aria-label={`${count} out of 5 stars`}>
...
// ─── Component ────────────────────────────────────────────────────────────────
export default function MoreSuccessStories({ pageData, successStories }) {
    ...
    <section className="w-full bg-[#FFFBF2] py-20 md:py-24">
      <div className="max-w-[1280px] mx-auto px-6 sm:px-4">
        ...
        <h2 className="text-[28px] md:text-[48px] font-light text-black text-center mb-10 sm:mb-15 leading-[1.2]">
          {headingLine1} <span className="font-bold">{headingLine2}</span>
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 sm:grid-cols-2 gap-6 md:gap-5 sm:gap-4">
          {stories.map((review) => (
            ...
            >
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0">
                    <h3 className="text-[#0f172a] text-base font-bold m-0 mb-1 leading-[1.3]">{review.name}</h3>
                    <span className="text-[#94a3b8] text-[13px] block">{review.time}</span>
                  </div>
                </div>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
              ...
              <p className="text-[#475569] text-[15px] leading-relaxed m-0 line-clamp-4">{review.text}</p>
