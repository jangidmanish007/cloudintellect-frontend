...
export default function PlacementHero({ hero }) {
  const heroData = hero || {};
  const stats = heroData.stats && Array.isArray(heroData.stats) ? heroData.stats : DEFAULT_STATS;

  // Use a single background image similar to SalesforceDeveloperHero
  const bgImageUrl = `${process.env.NEXT_PUBLIC_IMG_PATH}images/gallery/placement-bg-image.webp`;
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 max-w-full sm:max-w-[480px] w-full">
        {stats.map((stat, i) => (
          <motion.div
            ...
            className="bg-[#0000003D] rounded-[14px] p-4 backdrop-blur-[2px] sm:p-5 md:p-6 flex flex-col gap-1.5 sm:gap-2 border border-[#FFFFFF1A] transition-all duration-300 hover:bg-[rgba(0,0,0,0.15)] hover:border-white/10"
          >
            <span className="font-excon text-[32px] sm:text-[40px] md:text-[48px] font-bold text-white leading-none">
              {stat.value}
            </span>
  ...
  return (
    <section className="w-full bg-[#0b1c33] overflow-hidden">
